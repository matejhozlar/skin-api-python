...
class SkinApiClient:
    """Synchronous client for the Createrington Skin API.
        ...
        while True:
            try:
                ...
                if attempt < self._retries:
                    time.sleep(retry_delay_seconds(0, None, attempt))
                    attempt += 1
                    continue
                raise SkinApiError(
            ...
            if is_retryable_status(status) and attempt < self._retries:
                time.sleep(retry_delay_seconds(status, body, attempt))
                attempt += 1
                continue
    ...
    def __enter__(self) -> "SkinApiClient":
        return self
